...
const cities = [
    ...
    "Kochi", "Thiruvananthapuram", "Lucknow", "Varanasi", "Guwahati",
    "Bhubaneswar", "Chandigarh", "Srinagar", "Port Blair", "Leh"
];
            ...
            flightCard.innerHTML = `
                <div class="airline-info">
                    ...
                    <div>
                        ...
                        ${flight.stops === 0 ? '<span class="airline-badge">Direct</span>' : ''}
                    </div>
                </div>
                <div class="flight-details">
                    <div class="flight-time">${flight.departureTime} → ${flight.arrivalTime}</div>
                    <div class="flight-duration">${flight.duration} ${flight.stops ? '• ' + flight.stops + ' stop' : '• Non-stop'}</div>
                </div>
                <div class="price-section">
                    <div class="price-breakdown">
                        <div class="flight-price">₹${flight.price}</div>
                        <div class="price-details">
                            <div>Base Fare: ₹${(flight.price * 0.7).toFixed(0)}</div>
                            <div>Taxes & Fees: ₹${(flight.price * 0.3).toFixed(0)}</div>
                            <hr>
                            <div><strong>Total: ₹${flight.price}</strong></div>
                        </div>
                    </div>
                    <button class="select-flight-btn" onclick="selectFlight('${flight.flightNumber}')">Select</button>
                </div>
            `;
...
function generateSeatMap() {
    ...
    for (let i = 0; i < 30; i++) {
        ...
        seat.setAttribute('data-seat', `${Math.floor(i/6) + 1}${String.fromCharCode(65 + (i%6))}`);
...
function showConfirmation() {
    ...
    document.getElementById('departureCity').textContent = document.getElementById('departure').value;
    document.getElementById('arrivalCity').textContent = document.getElementById('arrival').value;
    ...
    document.getElementById('departureDate').textContent = document.getElementById('departureDate').value;
    document.getElementById('flightNumber').textContent = flight.flightNumber;
    document.getElementById('cabinClass').textContent = document.getElementById('cabinClass').value;
    ...
    document.getElementById('fareSummary').innerHTML = `
        <p>Base Fare (${totalSeats} passenger${totalSeats > 1 ? 's' : ''}): ₹${basePrice * totalSeats}</p>
        <p>Seat Charges: ₹${seatPrice * totalSeats}</p>
        <p>Taxes & Fees: ₹${taxes}</p>
    `;
    document.getElementById('totalPaid').textContent = total.toLocaleString('en-IN');
    ...
    for(let i = 1; i <= children; i++) {
        passengerList += `<p>Child ${i} - Seat ${selectedSeats[adults+i-1] || 'NA'}</p>`;
    }
