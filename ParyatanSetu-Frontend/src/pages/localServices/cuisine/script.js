  ...
  fetch(`http://192.168.31.252:5001/form/cuisine?city=${searchTerm}`, {
    ...
    .then((data) => {
      ...
      const providers = data.data.map((provider) => ({
        ...
        type: "Local Restaurant",
        cuisine: ["Goan", "Indian"],
        foodType: provider.foodType,
        rating: 4.3,
        priceRange: "₹300-800",
        pricePerMeal: "₹400",
        image: "https://via.placeholder.com/150",
        speciality: provider.specialization,
        openingHours: provider.openingTime + " - " + provider.closingTime,
        amenities: provider.amenities,
        popularDishes: provider.foodItems,
      ...
      (place) => `
        <div class="place-card">
            ...
            <div class="place-info">
                ...
                <p class="food-type">Food Type: ${place.foodType}</p>
                <p class="rating">Rating: ⭐ ${place.rating}</p>
                <p class="price">Price Range: ${place.priceRange}</p>
                <p class="meal-price">Average Meal Cost: ${
                  place.pricePerMeal
                }</p>
